...
def test(epoch):
    ...
    acc = AverageMeter(100)
    acc2s = AverageMeter(100)
    acc3s = AverageMeter(100)
    correct = 0
    total = 0
    for batch_idx, (inputs, targets) in enumerate(testloader):
        ...
        with torch.no_grad():
            ...
            acc.update(100. * correct_idx / float(targets.size(0)))
            acc2s.update(float(acc2))
            acc3s.update(float(acc3))
    ...
    logger.info("acc2: {}".format(acc2s.avg))
    logger.info("acc3: {}".format(acc3s.avg))
