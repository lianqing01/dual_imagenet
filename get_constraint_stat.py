...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        rema:in_time = '{:02d}:{:02d}:{:02d}'.format(int(t_h), int(t_m), int(t_s))
        mean_sum = 0
        var_sum = 0
        layer = 0
        ...
        for m in net.modules():
            if isinstance(m, Constraint_Norm):
                layer += 1
                mu = m.mu_.abs().mean().item()
                gamma = m.gamma_.abs().mean().item()
                mean_sum += mu
                var_sum += gamma
                tb_logger.add_scalar("batch_constraint_param/{:02d}_mu".format(layer), mu, curr_idx)
                tb_logger.add_scalar("batch_constraint_param/{:02d}_gamma".format(layer), gamma, curr_idx)
                wandb.log({"batch_constraint_param/{:02d}_mu".format(layer): mu,
                        "batch_step": curr_idx}, step=epoch)
                wandb.log({"batch_constraint_param/{:02d}_gamma".format(layer): gamma,
                        "batch_step": curr_idx}, step=epoch)

        tb_logger.add_scalar("batch_constraint_param/mu", mean_sum/float(layer), curr_idx)
        tb_logger.add_scalar("batch_constraint_param/gamma", var_sum/float(layer), curr_idx)

        wandb.log({"batch_constraint_param/mu".format(layer): mean_sum/float(layer),
                "batch_step": curr_idx}, step=epoch)
        wandb.log({"batch_constraint_param/gamma".format(layer): var_sum/float(layer),
                "batch_step": curr_idx}, step=epoch)
        ...
        for m in net.modules():
            if isinstance(m, Constraint_Norm):
                layer += 1
                mu = m.pre_mean.abs().mean().item()
                gamma = m.pre_mean.abs().mean().item()
                mean_sum += mu
                var_sum += gamma
                tb_logger.add_scalar("batch/{:02d}_mean".format(layer), mu, curr_idx)
                tb_logger.add_scalar("batch/{:02d}_var".format(layer), gamma, curr_idx)
                wandb.log({"batch/{:02d}_mean".format(layer): mu,
                        "batch_step": curr_idx}, step=epoch)
                wandb.log({"batch/{:02d}_var".format(layer): gamma,
                        "batch_step": curr_idx}, step=epoch)
